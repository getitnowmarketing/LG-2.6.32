...
PACK(void *) diagpkt_alloc (diagpkt_cmd_code_type code, unsigned int length)
{
	...
	/*-----------------------------------------------
	  ...
	  -----------------------------------------------*/
	pdiag_data = (diag_data*)vmalloc (size);
	if(NULL == pdiag_data)
	{
...
void diagpkt_tbl_reg (const diagpkt_user_table_type * tbl_ptr)
{
	...
	bindpkt_params *bind_req = (bindpkt_params*)kmalloc(sizeof(bindpkt_params)*num_entries,
			GFP_KERNEL);
	...
	if(NULL != bind_req)
	{
		...
		printk(KERN_ERR "%s bind_req is NULL\n", __FUNCTION__);
	}
}
...
void diagpkt_commit (PACK(void *)pkt)
{
	...
	if (pkt) {
...
#endif
		length = DIAG_REST_OF_DATA_POS + FPOS(diagpkt_lsm_rsp_type, rsp.pkt)
			+ item->rsp.length + sizeof(uint16);

		if (item->rsp.length > 0) {
			temp =  (unsigned char*) kmalloc((int)DIAG_REST_OF_DATA_POS
					+ (int)(item->rsp.length), GFP_KERNEL);
			memcpy(temp, (unsigned char*)&type, DIAG_REST_OF_DATA_POS);
			memcpy(temp+4, pkt, item->rsp.length);

#ifdef CONFIG_LGE_DIAG_DEBUG
			printk(KERN_INFO "LG_FW : printing buffer %d\n",
					(int)(item->rsp.length + DIAG_REST_OF_DATA_POS));

			for(i=0; i < (int)(item->rsp.length + DIAG_REST_OF_DATA_POS) ;i++)
				printk(KERN_INFO "0x%x ", ((unsigned char*)(temp))[i]);      

			printk(KERN_INFO "\n");
#endif
			/*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
			if(diagchar_write((const void*) temp, item->rsp.length + DIAG_REST_OF_DATA_POS)) {
				printk(KERN_ERR "LG_FW : Diag_LSM_Pkt: WriteFile Failed in diagpkt_commit\n");
				gPkt_commit_fail++;
			}
		}
		...
		printk(KERN_ERR "%s diagpkt is NULL\n", __FUNCTION__);
	}/* end if (pkt)*/
...
void diagpkt_commit_mtc(PACK(void *)pkt)
{
	...
	if (pkt) {
		...
		/* end mobile-view */
#ifdef CONFIG_LGE_DIAG_DEBUG
		printk(KERN_INFO "LG_FW: printing buffer at top\n");
		for(i=0;i<item->rsp.length;i++)
			printk(KERN_INFO "0x%x ", ((unsigned char*)(pkt))[i]);      
#endif
		...
		printk(KERN_ERR "%s diapkt is NULL\n", __FUNCTION__);
	}/* end if (pkt)*/
...
#ifdef CONFIG_LGE_DIAG_DEBUG
	printk(KERN_INFO "LG_FW: print received packet \n");
	for (i=0;i<pkt_len;i++) {
		printk(KERN_INFO "0x%x ",*((unsigned char*)(req_pkt + i)));
	}
	printk(KERN_ERR "\n");
#endif
