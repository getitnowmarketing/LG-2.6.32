...
/* LGE_CHANGES_S [mina@lge.com] 2009-11-20, [VS740] for hidden test menu */
bool sensorAlwaysOnTest = false;
#endif
/* LGE_CHANGES_E [mina@lge.com] 2009-11-20, [VS740] */
...
static int msm_get_stats(struct msm_sync *sync, void __user *arg)
{
	...
	default:
		...
		goto failure;
			...
			if(af_mode_started==1) // when autofucus started
			{
			...
			else if(af_mode_started==2) // when autofucus cancelled
			{
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_PREFLASH_VALUES:
		rc = pmsm->sync->sctrl.s_config(argp);
		break;

	case MSM_CAM_IOCTL_YUV_AF_CTRL_CANCEL:
		rc = pmsm->sync->sctrl.s_config(argp);
		af_mode_started = 2; //cancel autofocus
		break;
...
#if defined (CONFIG_MT9T111)
static int msm_get_flash_brightness(struct msm_sync *sync, void __user *arg)
{
	int rc = 0;
	int camera_flash_brightness;
	struct msm_camera_sensor_info *sdata;

/*
	if (copy_from_user(&camera_flash_brightness,
			arg,
			sizeof(int))) {
		ERR_COPY_FROM_USER();
		return -EFAULT;
...
*/
	...
	camera_flash_brightness = sync->sctrl.s_sf_cmd(CFG_GET_FLASH_BRIGHTNESS);
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_SENSOR_INFO:
		...
		break;
#if defined (CONFIG_MT9T111)
	case MSM_CAM_IOCTL_GET_FLASH_BRIGHTNESS:
		rc = msm_get_flash_brightness(pmsm->sync, argp);
		break;
#endif
	default:
		rc = msm_ioctl_common(pmsm, cmd, argp);
