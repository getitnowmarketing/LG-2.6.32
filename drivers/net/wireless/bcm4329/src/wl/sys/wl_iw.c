...
/* LGE_CHANGE_S, [dongp.kim@lge.com], 2010-09-08, <Disable setting power save mode if PM is 0> */
...
/* LGE_CHANGE_E, [dongp.kim@lge.com], 2010-09-08, <Disable setting power save mode if PM is 0> */

/* LGE_CHANGE_S, [dongp.kim@lge.com], 2010-09-08, <Disable setting power save mode if PM is 1> */
...
/* LGE_CHANGE_E, [dongp.kim@lge.com], 2010-09-08, <Disable setting power save mode if PM is 1> */
...
#if defined(CONFIG_LGE_BCM432X_PATCH)	//by sjpark 100827 : merge
...
#if defined(CONFIG_BRCM_LGE_WL_HOSTWAKEUP) && defined(CONFIG_BRCM_LGE_WL_ARPOFFLOAD)	//by sjpark 100827 : merge
static int
wl_iw_set_hostip(
	struct net_device *dev,
	struct iw_request_info *info,
...
)
{
	...
	#if defined(CONFIG_LGE_BCM432X_PATCH)	//by sjaprk 100828 : merge
...
#if defined(CONFIG_LGE_BCM432X_PATCH) && defined(CONFIG_BRCM_USE_DEEPSLEEP)	//by sjpark 100827 : merge
...
#else	/* defined(CONFIG_BRCM_USE_GPIO_RESET) */		//by sjpark 100827 : merge
...
#if defined(CONFIG_LGE_BCM432X_PATCH) && !defined(CONFIG_BRCM_USE_GPIO_RESET)		//by sjpark 100827 : merge
{
		scb_val_t scbval;
...
/* LGE_CHANGE_S, [yoohoo@lge.com], 2009-11-19, Use deepsleep instead of dhd_dev_reset when driver start or stop */
#if defined(CONFIG_BRCM_USE_DEEPSLEEP)		//by sjpark 100827 : merge
		/* Use Deep Sleep instead of WL Reset*/
...
#if defined(CONFIG_BRCM_USE_GPIO_RESET) && !defined(CONFIG_BRCM_USE_DEEPSLEEP)	//by sjpark 100827 : merge
#if defined(BCMLXSDMMC)
		sdioh_stop(NULL);
#endif
...
)
{
	...
	switch (nchan) {
	//case 11: strcpy(buf, "US"); break; //yangseon.so 2010.11.15
...
//		ret = wl_iw_control_wl_on(dev, info);		//hyeok
		if (strnicmp(extra, "START", strlen("START")) != 0){
			...
			if(extra)
				kfree(extra);
			// byoungwook.baek@lge com patch wake lock bug-2010.07.30 [START]
			WAKE_UNLOCK(iw->pub, WAKE_LOCK_PRIV);
			WAKE_LOCK_DESTROY(iw->pub, WAKE_LOCK_PRIV);
			// byoungwook.baek@lge com patch wake lock bug-2010.07.30 [END]
			return -EFAULT;
#endif
		}
		else {
...
#endif /* CONFIG_LGE_BCM432X_PATCH */	//by sjpark 100827 : merge
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-07-09, send wl_iw_send_priv_event only if receiving regular START command */		
