...
/* _S [hyunhui.park] 2010-08-12, Add Header for LGE USB */
...
/* S_S [younsuk.song] 2010-07-04, Add debugging macro sourced by hyunhui.park */
#ifdef CONFIG_USB_SUPPORT_LGE_ANDROID_GADGET

/* _S [hyunhui.park] 2010-04-08, Macro for debugging */
/* Debug mask value
...
#endif
/* _E [hyunhui.park] 2010-04-08 */
/* S_E [younsuk.song] 2010-07-04 */
...
/* _S [hyunhui.park] 2010-08-12, Detect Cable Type */
...
/* _S [hyunhui.park] 2010-07-15, Detect TA from ARM9 */
...
/* _S [hyunhui.park] 2010-08-12, Factory Cable Sleep Skip */
...
/* S_S [younsuk.song] 2010-06-21, [VS660] Support Factory USB Cable sourced by VS760*/
#ifdef CONFIG_USB_SUPPORT_LGE_FACTORY_USB
		...
		case USB_STATE_OFFLINE:
			...
			if ((flags & USB_FLAG_VBUS_ONLINE) && _vbus) {
				...
				schedule_delayed_work(
						&ui->chg_det,
						USB_CHG_DET_DELAY);
				/* ALESSI : _S [euikyeom.kim] 2010-08-09, Change USB state */
				/* 2009-12-29, To send wakeup uevent, we use switch device regardless of USB
				 * driver on Windows. If AC power (such as TA adapter) is connected instead
				 * of USB cable, we change switch state to 2 (powered).
				 */
				switch_set_state(&ui->sdev, 2);
				pr_info("HSUSB(%s) : switch_set_state(2)\n", __func__);
				/* _E [euikyeom.kim] 2010-08-09 */
			}
			else if(flags & USB_FLAG_VBUS_OFFLINE)
			{
				/* ALESSI : _S [euikyeom.kim] 2010-08-09, Change USB state */
				/* 2009-12-29, To send wakeup uevent, we use switch device regardless of USB
				 * driver on Windows. If AC power (such as TA adapter) is connected instead
				 * of USB cable, we change switch state to 2 (unpluged powered).
				 */
				switch_set_state(&ui->sdev, 3);
				pr_info("HSUSB(%s) : switch_set_state(3)\n", __func__);
				/* _E [euikyeom.kim] 2010-08-09 */
			}
			break;
		}
...
// LGE_UPDATE_S : Alessi Only	
	mdelay(100);
// LGE_UPDATE_E : Alessi Only; hongwon.lee  12/08/10: Timing difference between P500 and E720 to initialize usb driver
...
/* _S [younsuk.song] 2010-08-24, QCT patch for USB kernel panic */
	if (dev->irq) { 
		printk("freeing IRQ\n"); 
		free_irq(dev->irq, dev); 
		dev->irq = 0; 
	} 
/* _E [younsuk.song] 2010-08-24 */

	dev->state = USB_STATE_IDLE;
	atomic_set(&dev->configured, 0);
	switch_set_state(&dev->sdev, 0);

/* _S [hyunhui.park] 2010-08-14, From Function driver  */
//#ifdef CONFIG_USB_SUPPORT_LGE_GADGET_GSM
