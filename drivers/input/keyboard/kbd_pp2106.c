...
static void pp2106_fetchkeys(struct work_struct *work)
{
  ...
  int ret;
	...
	spin_lock_irqsave(&pp2106_spin_lock, flags);

  if(gpio_get_value(pp2106_pdata->irq_pin) == 1)
  {
		spin_unlock_irqrestore(&pp2106_spin_lock, flags);
    return;
  }
	ret = pp2106_get_data(&buf);
	spin_unlock_irqrestore(&pp2106_spin_lock, flags);
  if(ret == 1)
  {
		printk(KERN_INFO"Keypad : read fail\n"); 
    pp2106_hwreset();
    return;
  }
  else if(ret == 2)
  {
		printk(KERN_INFO"Keypad : Interrupt signal timeout\n"); 
    return;
  }
...
{
	//printk(KERN_INFO"Keypad suspend\n");
  QWERTY_SDA_INPUT();
	udelay(25);
	if(!QWERTY_SDA_READ() || (gpio_get_value(pp2106_pdata->irq_pin) == 0)) {
		printk(KERN_INFO"Keypad error detected qwerty_hw_reset\n");
		QWERTY_SDA_HIGH();
		QWERTY_SCL_HIGH();
    pp2106_hwreset();
  }
	/* future capability*/
#if defined(CONFIG_MACH_MSM7X27_ALOHAG)
	/* FIXME : following code is due to H/W bug of rev.A
	 * 2010-01-29, cleaneye.kim@lge.com
	 */
	disable_irq_wake(MSM_GPIO_TO_INT(pp2106_pdata->irq_pin));
#endif
	return 0;
}
...
static int pp2106_resume(struct platform_device *pdev)
{
	 ...
	 * 2010-01-29, cleaneye.kim@lge.com
	 */
	enable_irq_wake(MSM_GPIO_TO_INT(pp2106_pdata->irq_pin));
...
/* LGE_CHANGE_S [khlee.lee@lge.com] 2010-02-24, LG_FW_UTS */
/* CTS test needs to access directly star and pound key */
	...
	set_bit(keycode, pp2106_kbd_dev->keybit);
#endif
/* LGE_CHANGE_E [khlee.lee@lge.com] 2010-02-24, LG_FW_UTS */
	...
	spin_lock_init(&pp2106_spin_lock);
