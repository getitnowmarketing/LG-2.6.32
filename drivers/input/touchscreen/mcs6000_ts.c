...
static void mcs6000_ts_work_func(struct work_struct *work)
{
	...
	struct mcs6000_ts_data *ts
		= container_of(to_delayed_work(work), struct mcs6000_ts_data, work);
	...
	if (ts->pendown) { /* touch pressed case */
		...
		if (touch_pressed) {
			...
			touch_pressed = 0; /* update touch status */
		}
	}
...
void mcs6000_firmware_info(unsigned char* fw_ver, unsigned char* hw_ver)
{
	...
	struct vreg *vreg_touch = vreg_get(0, "ruim");
#else
...
EXPORT_SYMBOL(mcs6000_firmware_info);

static __inline int mcs6000_ioctl_down_i2c_write(struct file *file, 
...
static __inline int mcs6000_ioctl_down_i2c_read(struct file *file, 
		unsigned char addr, unsigned char *ret)
{
	struct mcs6000_ts_data *ts = file->private_data;
	int err = 0;
	struct i2c_msg msg;
...
int mcs6000_ts_ioctl_down(struct inode *inode, struct file *file, 
...
static int mcs6000_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* jinkyu.choi@lge.com
	 * sometimes, the touch firmware is not available,
	 * in case of this, the touch firmware should be updated at auto upgrade routine.
	 * the following routines result in touch device probing fail.
	 * the touch firmware can not be updated by auto upgrade routine.
	 */
#if 0
	ret = i2c_smbus_write_byte_data(ts->client, 0x1e, 0x01);	/* device reset */
	if (ret < 0) {
	...
	ret = i2c_smbus_write_byte_data(ts->client, 0x1d, 0x00);
	if (ret < 0) {
		printk(KERN_ERR "mcs6000_ts_probe: disable interrupt, i2c write failed\n");
		goto err_detect_failed;
	}
#endif
