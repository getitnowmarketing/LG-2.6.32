...
#define HS_FLIP_OPEN_K	0x9C	/* multikit detection */
#endif
...
#endif
#ifdef CONFIG_MACH_MSM7X27_ALOHAV
	KEY(HS_FLIP_OPEN_K, HS_FLIP_OPEN_K),
#endif
	0
};
...
#ifdef CONFIG_MACH_MSM7X27_ALOHAV
extern void hall_ic_dock_report_event(int);
#endif
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	case SW_HEADPHONE_INSERT:
		...
		break;
#ifdef CONFIG_MACH_MSM7X27_ALOHAV
	case HS_FLIP_OPEN_K: /* MultiKit Detect */
		if(key_parm == HS_FLIP_OPEN_K) // inserted
			hall_ic_dock_report_event(1); // DOCK_STATE_DESK
		else // removed
			hall_ic_dock_report_event(0);  // DOCK_STATE_UNDOCKED
		return ;
#endif
	case -1:
