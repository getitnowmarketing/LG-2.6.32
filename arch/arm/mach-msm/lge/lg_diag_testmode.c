...
#if CONFIG_TOUCHSCREEN_MCS6000
#include <mach/pmic.h>

bool tsdown_done = FALSE;
extern int pmic_secure_mpp_config_i_sink(enum mpp_which,
		enum mpp_i_sink_level, enum mpp_i_sink_switch);

static int tsdown_app_execute(void)
{
	int ret = 0;
	char cmdstr[100];
	int fd;
	...
	if ( (fd = sys_open((const char __user *) "/system/bin/tsdown", O_RDONLY ,0) ) < 0 ) {
		return ret;
	} else {
		printk("\n execute /system/bin/tsdown\n");
		sprintf(cmdstr, "/system/bin/tsdown\n");
		sys_close(fd);
	}
	printk(KERN_INFO "execute - %s", cmdstr);
	if ((ret = call_usermodehelper("/system/bin/sh", argv, envp, UMH_WAIT_PROC)) != 0) {
		printk(KERN_ERR "tsdown failed to run \": %i\n",
				ret);
	} else
		printk(KERN_INFO "tsdown execute ok");

	return ret;
}

extern void mcs6000_firmware_info(unsigned char*, unsigned char*);
#endif
...
void* LGF_TestModeVer(	test_mode_req_type* pReq ,DIAG_TEST_MODE_F_rsp_type	*pRsp)
{
	...
	if(pReq->version == TEST_MODE_TOUCH_HW_VERSION) {   
		mcs6000_firmware_info(&fw_ver, &hw_ver);
		sprintf(rsp_buf,"%x",hw_ver);
		strcpy((char *)pRsp->test_mode_rsp.str_buf ,rsp_buf); 
		pRsp->ret_stat_code = TEST_OK_S;
		tsdown_done = FALSE;
	} else if(pReq->version == TEST_MODE_TOUCH_FW_VERSION) {
		mcs6000_firmware_info(&fw_ver, &hw_ver);
		sprintf(rsp_buf,"%x",fw_ver);
		strcpy((char *)pRsp->test_mode_rsp.str_buf ,rsp_buf); 
		pRsp->ret_stat_code = TEST_OK_S;
		tsdown_done = FALSE;
	} else if(pReq->version == TEST_MODE_TOUCH_FW_UPDATE) {
		if(tsdown_done == FALSE) {
			pmic_secure_mpp_config_i_sink(PM_MPP_15,
