...
int  lge_set_hidden_enable(int enable)
{
       int err;
	err = msm_proc_comm(PCOM_CUSTOMER_CMD3, &enable, 0 );
	if (err < 0) {
		pr_err("%s: msm_proc_comm(PCOM_CUSTOMER_CMD3) failed\n",
		       __func__);
		return err;
	}
	return enable;
}
EXPORT_SYMBOL(lge_set_hidden_enable);
#endif
#endif
//LGE_UPDATE_E
