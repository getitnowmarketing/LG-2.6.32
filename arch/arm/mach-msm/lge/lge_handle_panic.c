...
//LGE_UPDATE_S/sunkyo.park/hidde reset patch for ALESSI
#ifdef CONFIG_MACH_MSM7X27_ALESSI
static int hidden_reset_param_set_int(const char *val, struct kernel_param *kp);

#define lge_module_param_named(name, value, type, perm)   param_check_##type(name, &(value));   module_param_call(name, hidden_reset_param_set_int, param_get_##type, &value, perm);   __MODULE_PARM_TYPE(name, #type)
int hidden_reset_enable = 0;
lge_module_param_named(
...
MODULE_AUTHOR("SungEun Kim <cleaneye.kim>");
MODULE_LICENSE("GPL");
