        ...
        uint16		pkt_len )		      /* length of request packet   */
{
  ...
  /*
   * 2010-12-10, clear all call logs, jinkyu.choi@lge.com
   * the following is for VS740 Froyo UI
   * the values should be modified with the target Model UI
   */
  switch (keycode) {
	  case 0x60: /* select call log menu */ 
		  if( is_slide_open() == 0)
			  Send_Touch(400, 300);
		  else
			  Send_Touch(180, 85);
		  break;
	  case 0x61: /* select Menu */
		  SendKey(KEY_MENU, 0);
		  break;
	  case 0x62: /* clear call log */
		  if( is_slide_open() == 0) 
			  Send_Touch(50, 600);
		  else
			  Send_Touch(360, 750);
