...
static int ctrl_prio_suspend_kthread(int policy)
{
	...
	switch (policy) {
		case SCHED_NORMAL:
			rc = sched_setscheduler_nocheck(current, SCHED_NORMAL, &param_normal);
			printk(KERN_INFO"%s: %d\n",__func__, rc);
			yield();
			break;
		case SCHED_FIFO:
			rc = sched_setscheduler_nocheck(current, SCHED_FIFO, &param_fifo);
			break;
		default :
			break;
	}
...
static int lge_pm_ctrl_prio_normal(void)
{
	return ctrl_prio_suspend_kthread(SCHED_NORMAL);
}

static void lge_pm_ctrl_prio_fifo(void)
{
	ctrl_prio_suspend_kthread(SCHED_FIFO);
	return;
}
#endif
...
#ifdef CONFIG_LGE_SUSPEND_HIGH_PRIO_PATCH
	.setup_prio = lge_pm_ctrl_prio_normal,
	.wake = lge_pm_ctrl_prio_fifo,
#endif
	.enter = msm_pm_enter,
...
#ifdef CONFIG_MACH_MSM7X27_ALOHAV
	/*
	 * 2010-12-15, jinkyu.choi@lge.com
	 * in case of VS740,
	 * just only, make the last_kmsg when kernel panic occured.
	 */
	static struct ram_console_buffer *ram_console_buffer = 0;

	ram_console_buffer = get_ram_console_buffer();
	ram_console_buffer->sig = 0x00000000;
#endif
