 ...
 // _S [bluerti]
	if (lge_error_analyzing == 1){
	  ...
	  if(lge_error_fb_cnt < LGE_ERROR_MAX_ROW)
	  {
	     ...
	     snprintf(crash_buf[lge_error_fb_cnt], LGE_ERROR_MAX_COLUMN, "[<%08lx>]%s", sym1 ); 
	     lge_error_fb_cnt++;
	  }
	  //LGE_UPDATE_E
	} else {	// Normal case 
...
static void __die(const char *str, int err, struct thread_info *thread, struct pt_regs *regs)
{
	...
	if (!hidden_reset_enable) {
		...
		dump_backtrace(regs, tsk);
		dump_instr(KERN_EMERG, regs);
		//smsm_reset_modem(SMSM_APPS_SHUTDOWN); //[blue.park] It will be informed a kernel panic to Modem side.
